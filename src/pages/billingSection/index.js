  ...
  const getUserInfo = async () => {
    try {
      ...
      const response = await axios.post(
        "http://3.132.248.171:4500/get_user_details",
        formData
      );
    ...
    <div className="billing-container">
      ...
      <div className="billing-box  ">
        <div className="billing-info">
          <p className="billing-header">You are currently on a " Plan"</p>
          <p>You have credits left</p>
        </div>
        <div className="billing-buttons">
          <button className="upgrade-btn" onClick={() => navigate("/pricing")}>
            Upgrade your plan
          </button>
          <button className="stop-btn btn">Stop your plan</button>
        </div>
      </div>
      <div className="design-prompt">
        <p>Are you ready to transform your space?</p>
        <button
          className="design-btn btn"
